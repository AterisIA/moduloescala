...
serve(async (req) => {
  ...
  try {
    const { nome, matricula, id_contato_terceirizacao, latitude, longitude, images } = await req.json();

    console.log('[Enroll] Recebendo cadastro:', { nome, matricula, id_contato_terceirizacao, latitude, longitude, imageCount: images?.length });
    ...
    // Usar Gemini apenas para descrição (comparação será feita diretamente)
    const LOVABLE_API_KEY = Deno.env.get('LOVABLE_API_KEY');
    if (!LOVABLE_API_KEY) {
      throw new Error('LOVABLE_API_KEY não configurada');
    }

    console.log('[Enroll] Analisando características faciais...');
        ...
        messages: [
          {
            ...
            content: [
              {
                type: 'text',
                text: 'Analise estas 3 fotos da mesma pessoa e descreva as características faciais: formato do rosto, olhos, nariz, boca, sobrancelhas, cabelo, marcas distintivas.'
              },
              ...images.map(img => ({
                type: 'image_url',
                image_url: { url: img }
    ...
    if (!aiResponse.ok) {
      ...
      throw new Error('Erro ao analisar características');
    }
    ...
    // Salvar cadastro (comparação será feita diretamente pelas imagens)
    const { data, error } = await supabase
      ...
      .insert({
        ...
        id_contato_terceirizacao: id_contato_terceirizacao || null,
        latitude: latitude,
        longitude: longitude,
