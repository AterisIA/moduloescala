-- Atualizar função para usar resposta_comunicacao
CREATE OR REPLACE FUNCTION public.get_quadrant_counts_with_hours(p_filter text, p_start date, p_end date)
RETURNS TABLE(entity_id uuid, entity_name text, dt date, presenca integer, atraso integer, falta integer, fj_at integer, presenca_horas numeric, atraso_horas numeric, falta_horas numeric, fj_at_horas numeric)
LANGUAGE plpgsql
SECURITY DEFINER
AS $function$
BEGIN
  IF p_filter = 'Coordenadores' THEN
    ...
    SELECT
      ...
      COUNT(DISTINCT CASE WHEN rc.status::text = '1' THEN e.idescala END)::int AS presenca,
      COUNT(DISTINCT CASE WHEN rc.status::text = '2' THEN e.idescala END)::int AS atraso,
      COUNT(DISTINCT CASE WHEN rc.status::text = '3' THEN e.idescala END)::int AS falta,
      COUNT(DISTINCT CASE WHEN rc.status::text IN ('4', '5') THEN e.idescala END)::int AS fj_at,
      SUM(CASE 
        WHEN rc.status::text = '1' THEN 
          EXTRACT(EPOCH FROM (COALESCE(e.finalescala, e.dataescala) - e.dataescala)) / 3600.0
        ELSE 0 
      END)::numeric AS presenca_horas,
      SUM(CASE 
        WHEN rc.status::text = '2' THEN 
          EXTRACT(EPOCH FROM (COALESCE(e.finalescala, e.dataescala) - e.dataescala)) / 3600.0
        ELSE 0 
      END)::numeric AS atraso_horas,
      SUM(CASE 
        WHEN rc.status::text = '3' THEN 
          EXTRACT(EPOCH FROM (COALESCE(e.finalescala, e.dataescala) - e.dataescala)) / 3600.0
        ELSE 0 
      END)::numeric AS falta_horas,
      SUM(CASE 
        WHEN rc.status::text IN ('4', '5') THEN 
          EXTRACT(EPOCH FROM (COALESCE(e.finalescala, e.dataescala) - e.dataescala)) / 3600.0
        ELSE 0 
